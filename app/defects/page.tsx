...
export default function DefectsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.reportedBy?.trim()) {
      showAlert(
        "Validation Error",  
        "Please enter who reported this defect",
        "warning",
      );
      return;
    }
    ...
    const selectedMaterial = materials.find(
      (m) => m.materialCode === formData.materialCode,
    );
    ...
    const defect: Defect = {
      ...
      materialDescription:
        formData.materialDescription || selectedMaterial?.description || "",
  ...
  return (
    <div className={`min-h-screen ${bgMain} transition-colors duration-300`}>
      ...
      {/* Header */}
      <div
        className={`${bgHeader} border-b ${borderColor} px-8 py-6 transition-colors duration-300`}
      >
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <h1
              className={`text-xl font-semibold ${textPrimary} transition-colors duration-300`}
            >
              Defects Module
            </h1>
            <p
      ...
      <div className="p-8 space-y-6">
        <div
          className={`${bgCard} rounded-lg border ${borderCard} p-4 transition-colors duration-300`}
        >
          <div className="relative">
            <Search
              className={`absolute left-4 top-1/2 transform -translate-y-1/2 ${textSecondary} transition-colors duration-300`}
              size={20}
            />
            <input
              type="text"
              ...
              className={`w-full pl-12 pr-4 py-3 border ${borderCard} rounded-xl focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500/50 transition-all ${
                theme === "dark" ? "bg-slate-900/50" : "bg-slate-50"
              } ${textPrimary} placeholder-slate-500 backdrop-blur-sm transition-colors duration-300`}
            />
          </div>
        </div>

        <div
          className={`${bgCard} rounded-lg border ${borderCard} overflow-hidden transition-colors duration-300`}
        >
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1200px]">
              <thead className={bgTableHeader}>
                <tr>
                  <th
                    className={`px-4 py-4 text-left text-xs font-medium ${textTertiary} uppercase tracking-wider w-32 transition-colors duration-300`}
                  >
                    Date
                  </th>
                  <th
                    className={`px-4 py-4 text-left text-xs font-medium ${textTertiary} uppercase tracking-wider w-40 transition-colors duration-300`}
                  >
                    Material Code
                  </th>
                  <th
                    className={`px-4 py-4 text-left text-xs font-medium ${textTertiary} uppercase tracking-wider min-w-[200px] transition-colors duration-300`}
                  >
                    Description
                  </th>
                  <th
                    className={`px-4 py-4 text-left text-xs font-medium ${textTertiary} uppercase tracking-wider w-36 transition-colors duration-300`}
                  >
                    Defect Type
                  </th>
                  <th
                    className={`px-4 py-4 text-left text-xs font-medium ${textTertiary} uppercase tracking-wider w-28 transition-colors duration-300`}
                  >
                    Quantity
                  </th>
                  <th
              ...
              <tbody
                className={`${bgCard} ${divideColor} transition-colors duration-300`}
              >
                {isLoading ? (
                  <tr>
                    ...
                    <td colSpan={9} className="px-6 py-12 text-center">
                      <div className="flex flex-col items-center">
                        <div
                          ...
                          } rounded-full mb-4 transition-colors duration-300`}
                        >
                          <AlertTriangle className={textSecondary} size={32} />
                        </div>
                        <p
                          className={`${textTertiary} font-medium mb-1 transition-colors duration-300`}
                        >
                          No defects found
                        </p>
                  ...
                  filteredDefects.map((defect) => (
                    <tr
                      key={defect.id}
                      className={`${hoverRow} transition-colors group`}
                    >
                      <td
                        className={`px-4 py-4 whitespace-nowrap text-sm ${textSecondary} transition-colors duration-300`}
                      >
                        {new Date(defect.reportedDate).toLocaleDateString()}
                      </td>
                      <td
                        className={`px-4 py-4 whitespace-nowrap text-sm font-medium ${textPrimary} transition-colors duration-300`}
                      >
                        {defect.materialCode}
                      </td>
                      <td
                        className={`px-4 py-4 text-sm ${textTertiary} transition-colors duration-300`}
                        title={defect.materialDescription}
                      >
                        <div
                      ...
                      <td
                        className={`px-4 py-4 whitespace-nowrap text-sm ${textTertiary} transition-colors duration-300`}
                      >
                        {defect.defectType}
                      </td>
                      <td
                        className={`px-4 py-4 whitespace-nowrap text-sm font-semibold ${textPrimary} transition-colors duration-300`}
                      >
      ...
      {isModalOpen && (
        <div
          ...
          } backdrop-blur-sm flex items-center justify-center z-50 p-4 transition-colors duration-300`}
        >
          <div className="premium-card p-8 w-full max-w-md max-h-[90vh] overflow-y-auto custom-scrollbar animate-slide-in">
            <h2
              className={`text-2xl font-bold ${textPrimary} mb-6 transition-colors duration-300`}
            >
